...
export const parseCookiesFromHeaders = (headers: Headers): CookiesFromHeadersType => {
  const getSetCookie = headers.getSetCookie()
  const cookieList = getSetCookie.map((x) => x.split(';')[0])
  const jwt = cookieList.find((x) => x.includes(JWT))?.split('=')[1] ?? ''
  const userId = cookieList.find((x) => x.includes(USER_ID))?.split('=')[1] ?? ''
  return { jwt, userId }
}

export const formatCookiesForHeader = (cookiesStore: ReadonlyRequestCookies) => {
